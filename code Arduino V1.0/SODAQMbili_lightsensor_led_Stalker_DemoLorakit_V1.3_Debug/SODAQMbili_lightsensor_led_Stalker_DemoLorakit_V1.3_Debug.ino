...
unsigned long ul_delay_loop = 120000;   // ERC7003 868 MHz 1% duty cycle   
unsigned long ul_default_delay = 120000;
...
const int PCB_LEDPIN=9;         //Connect the PCB LED L0 to Pin13, Digital 13
const int EXT_LEDPIN=8;        //Connect the external Green LED to Pin 12, Digital 12
...
/*************************** methods **********************************/
 ...
 void initXbeeDebugSerials(int xbee_rate, int debug_rate)
 {
  ...
  Serial1.begin(xbee_rate);       // the Bee baud rate on Software Serial Atmega
  delay(1000);
 } 
 ...
 void initXbeeNanoN8()
 {
  ...
  str_dummy = Serial1.readString();
  delay(1500);
        
  //Serial.println("Read module version (DevEUI) : ");          
  Serial1.print("ATV\n");    // Return module version, DevEUI (LSB first), Stack version.
  str_firmwareVer = Serial1.readString();
  Serial.println(str_firmwareVer);
  delay(1500);
  
// /***************** OTAA ****************************************/
//  Serial.println(" --> OTAA ");    
//  Serial1.print("ATO83=3F\n");    // OTAA and Rx windows active 
//  str_dummy = Serial1.readString();
//  Serial.println(str_dummy);
//  delay(1500);

  
  /***************** Rx ****************************************/
  Serial.println(" --> Xbee Serial rate 19200 ATM007=06: ");    
  Serial1.print("ATM007=06\n");    // Baud rate 19200
  str_dummy = Serial1.readString();
  Serial.println(str_dummy);
  delay(1500);

  /***************** AppsKey ****************************************/
  Serial.println(" get appsKey ATO072: ");    
  Serial1.print("ATO072\n");    // Baud rate 19200
  str_dummy = Serial1.readString();
  Serial.println(str_dummy);
  delay(1500);

//  /***************** ATF ****************************************/
//  Serial.println(" return to factory config ");    
