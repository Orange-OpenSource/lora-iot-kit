 ...
 * pins SCL & SDA are connected respectively to analog 5 and 6 
 * sensor 1.2 groove temp 1.2 use only Data -> analog 5 (SCL), 5v -> 5v, GND -> gnd
*/ 
/***************************************************************/
 ...
 *   Last modified 05 july 2016 MAM Orange Labs (City)
 */
...
// **************** const delays *************************
const unsigned long ul_default_delay = 130600; // <=> 1% duty cycle, SF12, 5 bytes
...
void led_blinking(int sec)
 ...
 void sendLightSensorValuetoNanoN8(unsigned long l_value)
  { 
    if (ul_delay_loop == ul_default_delay)    // ERC7003 868 MHz 1% duty cycle frame  
    {
        ...
        for (int i = sizeof(l_value) - 1; i > -1 ; --i) //new light value transmit to NANO-N8
          SerialDebug.print((l_value >> (i << 3)) & 0xFF, HEX); //send in reverse endian
        SerialDebug.print(" ");
        
        //send the frame
        SerialXBee.write(pcbState); //LED state
        for (int i = sizeof(l_value) - 1; i > -1 ; --i) //new light value transmit to NANO-N8
          SerialXBee.write((l_value >> (i << 3)) & 0xFF); //send in reverse endian
    }
  }

 float RsensorCalc(int i_value)
  {
    float f_resistor = (1023 - i_value) * 10.0f / i_value;
    digitalWrite(LED_EXT_PIN, (f_resistor > LIGHT_THRESHOLD_VALUE) ? HIGH : LOW);
    return f_resistor;
  }
